//
//  FSPagerViewTransformerType.swift
//  MAZAADY_TASK
//
...
open class FSPagerViewTransformer: NSObject {
    ...
    open func applyTransform(to attributes: FSPagerViewLayoutAttributes) {
        ...
        case .overlap, .linear:
            ...
            let scale = max(1 - (1 - self.minimumScale) * abs(position), self.minimumScale + 0.11)
            
            let translationY: CGFloat = -attributes.bounds.height * 0.15 * position
